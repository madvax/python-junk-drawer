...
#    Select WEP, then select 64-bit encryption 
...
encryptionKey = "abcdef0123"  # Default 128-bit encryption key 
...
SUMARY: Sets up a DD-WRT 3.0 Access Point to use WEP 64-bit encryption 
...
DEFINED EXIT CODES:
  ...
  4 - Bad encryption key specified, must be 10 hex digits, no ':'s
   ...
   if arg[FIRST]== "-k" or arg[FIRST] == "--key":
      try: 
         if len(arg[LAST]) == 10:
            ...
            raise ValueError("Wrong size key, key length: %s, should be 10" %len(arg[LAST])) 
      except ValueError as e:
         sys.stderr.write("\nERROR -- Bad encryption key specified: \'%s\'\n         Valid key must have 10 hex digist, no ':' characters" %arg[LAST])
         sys.stderr.write("\n         %s\n\n" %str(e))
         sys.exit(4)          
...
if VERBOSE:
   sys.stdout.write("Selecting 64 Bit Encryption for WEP for %s interface  ... " %band)
   sys.stdout.flush()
try:
   encryptionDropdown = Select(driver.find_element_by_name( "wl%s_wep_bit" %interfaceId))
   encryptionDropdown.select_by_value("64")
   time.sleep(delay)
except Exception as e:
   sys.stderr.write("\nERROR -- Unable to find and/or click on \"Encryption (64-bit)\" pulldown for WEP")
   sys.stderr.write("\n         %s\n\n" %str(e))
   sys.exit(5)          
if VERBOSE: sys.stdout.write("done.\n")
...
if VERBOSE: 
   sys.stdout.write("Clearing 64-Bit Encryption key 1 for WEP for %s interface  ... " %(band))
   sys.stdout.flush()
try:
...
if VERBOSE:
   sys.stdout.write("Entering 64-Bit Encryption key 1 \"%s\" for WEP for %s interface  ... " %(encryptionKey,band))
   sys.stdout.flush()
try:
